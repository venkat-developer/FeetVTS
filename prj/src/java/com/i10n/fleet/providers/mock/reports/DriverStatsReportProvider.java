...
public class DriverStatsReportProvider implements IDataProvider {
	private static Logger LOG = Logger.getLogger(DriverStatsReportProvider.class);
	...
	public IDataset getDataset(RequestParameters params) {
		...
		if (null != params.getParameter(RequestParams.driverID)) {
			...
			if (avgSpeed != 0.0 && maxSpeed != 0.0 && distance != 0.0) {
				DriverReport driverreport = dr.fetchDriverReportAsEntity(driverId,browserTime,dateRange.getStart(), dateRange.getEnd());
				result.put("graphurl", driverreport.getImageName());
			} else {
