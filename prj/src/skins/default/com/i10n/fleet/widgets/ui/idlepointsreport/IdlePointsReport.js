(function() {
	...
	/**
	 * IdlePointsReport, a widget in vehicle report page,
	 * extends VehicleReport.ReportItem widget
	 *
	 * @author irk
	 */
	$W.IdlePointsReport = function(el, oArgs) {
		$W.IdlePointsReport.superclass.constructor.call(this, el, oArgs);
		this.init();
	};
	$L.extend($W.IdlePointsReport, $W.VehicleReport.ReportItem, {
		/**
			...
			if(column._nKeyIndex === 3 && JSON.parse(JSON.stringify(record))._oData.location.indexOf(":") != -1){
				...
				if(column._nKeyIndex == 4){
					...
					if (selectedTimeFrame && selectedVehicle) {
						URL = "/fleet/view/reports/?module=/blocks/json&data=view&report=vehiclemapreport";
					...
					if (this._widgets.items) {
						for (var itemKey in this._widgets.items) {
							if (itemKey != "vehiclemapreport") {
								this._widgets.items[itemKey].hide();
							}
						}
						this._widgets.items["vehiclemapreport"].show();
					}
					this.set($W.VehicleReport.ATTR_CURRENT_REPORT, "vehiclemapreport");
	...
	addExportButtonListener :  function(){
		...
		YAHOO.util.Event.addListener($D.getElementsByClassName("exportvehicleidlepoints"), "click",function(oArgs){
			var timeFrame= this.get($W.BaseReport.ATTR_SELECTED_TIMEFRAME);
			var pdf = document.getElementById("vehidlepointspdf").checked;
			var excel = document.getElementById("vehidlepointsexcel").checked;
			if(pdf || excel){
				if(this._oDataTable.getRecordSet().getRecords().length > 0){
					if(pdf && timeFrame ){
						...
						URL += "&reporttype=vehicleidlepoints" ;
	...
	addListenersForNextAndPrevious :  function(){
		...
		YAHOO.util.Event.addListener($D.getElementsByClassName("idlepointsnext"), "click",function(oArgs){
			var URL = null;
			var timeFrame= this.get($W.BaseReport.ATTR_SELECTED_TIMEFRAME);
		...
		YAHOO.util.Event.addListener($D.getElementsByClassName("idlepointsprevious"), "click",function(oArgs){
			this._oDataTable.get('paginator').setPage(this._oDataTable.get('paginator').getCurrentPage()-1);
			this.addListenersForAddressFetch();
		},null,this);
	},
	...
	render: function() {
		...
		if (this._oDataTable === null) {
			...
			this._oDataTable = new $W.IdlePointsReport.DataTable(this.elBase, config);

			vehMapReport=new $W.VehicleMapReport(this.elBase, config);
		...
		$U.Connect.asyncRequest('GET', "/fleet/view/reports/?module=/blocks/json&data=view&report=idlepointsreport"+
...
//			},null,this);
	...
	/**
	 * Constructs the config to be used by the datatable in IdlePointsReport
	 */
	 ...
	 * @param {Object} params
	 */
	$W.IdlePointsReport.DataTable = function(el, params) {
		this.initDataTable(el, params);
		this.initParams = params;
	};
	$L.extend($W.IdlePointsReport.DataTable, $W.Report, {
		/**
	...
	printReport: function(){
		...
		if (this.initParams) {
			...
			elPrintTitleContent.innerHTML = vehicleName + " 's  " + elPrintTitleContent.innerHTML;
	...
	$L.augmentObject($W.IdlePointsReport.DataTable.prototype, {
		/**
		...
		$W.IdlePointsReport.DataTable.superclass.constructor.call(this, el, config);
	},
	/**
