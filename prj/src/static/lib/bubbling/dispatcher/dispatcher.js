  ...
  */
  YAHOO.plugin.Dispatcher = function () {
	...
	*/
     ...
     * @return boolean
     */
    function show ( element, content, config ) {
        var doContinue = true;
        
        if ($L.isFunction(config.onShow)) {
            doContinue = config.onShow.apply ( config, [element, content, config] );
            doContinue = (undefined === doContinue) || doContinue;
        }
