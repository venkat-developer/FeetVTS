  ...
  */
  YAHOO.Bubbling = function () {
	...
	*/
     ...
     * @return void
     */
    obj.getActionNames = function (el, depot) {
        depot = depot || {};
        var b = null, r = null,
            f = ($D.inDocument(el)?function(b){return $D.hasClass(el, b)}:function(b){return el.hasClass(b);}); // f: check is certain object has a classname
        var result = [];
        ...
        if (!(args[1].flagged || args[1].decrepitate) || force) {
            ...
            if (behaviors) {
                ...
                for ( var i = 0; i < behaviors.length; i++ ) {
                    ...
                    if ( behavior && actions[behavior] && (actions[behavior].apply(args[1], [layer, args]))) {
                        // Flag for future behaviors
                        args[1].flagged = true;
                ...
                // If *any* of the behaviors stopped the event, force the YAHOO event stopped.
                if ( stopped ) {
                    $E.stopEvent(args[0]);
                }
            }
