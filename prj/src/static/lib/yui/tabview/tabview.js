...
(function() {
    ...
    YAHOO.extend(TabView, Y.Element, {
        ...
        /**
         * The className of the HTMLElement containing the TabView's tab content elements
         * to look for when building from existing markup, or to add when building
         * from scratch. 
         * All childNodes of the content container are treated as content elements when
         * building from existing markup.
         * @property TAB_CONTENT_CLASSNAME
         * @default "yui-content-tab"
         */
        TAB_CONTENT_CLASSNAME: 'yui-content-tab',
        ...
        addTab: function(tab, index) {
            ...
            if ( contentEl && !Dom.isAncestor(contentParent, contentEl) ) {
                if(!Dom.hasClass(contentEl,this.TAB_CONTENT_CLASSNAME)) {
                    Dom.addClass(contentEl,this.TAB_CONTENT_CLASSNAME)
                }
            ...
            this.setAttributeConfig(CONTENT, {
                ...
                method: function(value) {
                    ...
                    if (YAHOO.env.ua.ie <= 0) {
                        var children = this.get(CONTENT_EL).getElementsByTagName("script");
                        var scripts = [];
                        for (var i = 0; i < children.length; i++) {
                            var scriptTag = document.createElement('script');
                            scriptTag.appendChild(document.createTextNode(children[i].innerHTML));
                            scripts.push(scriptTag);
                        }
                        for (var i = 0; i < scripts.length; i++) {
                            this.get(CONTENT_EL).appendChild(scripts[i]);
                        }
                    }
                }
            });
