(function() {
     ...
     * @author sabarish
     */
    $W.SidePaneList = function(el, oArgs) {
        /**
    ...
    $L.augmentObject($W.SidePaneList, {
        ...
        listComparator: function(el1, el2) {
            var object1 = el1.getAttribute("itemname");
            var object2 = el2.getAttribute("itemname");
            var result = 0;
    ...
    $L.augmentObject($W.SidePaneList.prototype, {
        ...
        processTemplateData: function() {
          ...
          var sMarkupTemplate = "", selectTemplate = '<select class="item-group-select">';
         /* if(url.match('online')){
        	  selectTemplate= selectTemplate+'<option class="group-item" value="online-online selected">Moving</option>';              	  
          }else if(url.match('offline')){
        	  selectTemplate= selectTemplate+'<option class="group-item" value="offline-offline selected">Non tracking</option>';
          }else if(url.match('idle')){
        	  selectTemplate= selectTemplate+'<option class="group-item" value="idle-idle selected">Idle</option>';
          }else{*/
              selectTemplate = selectTemplate+'<option class="group-item" value="All" selected>All</option>';
        /*      selectTemplate= selectTemplate+'<option class="group-item" value="online-online">Moving</option>';
              selectTemplate= selectTemplate+'<option class="group-item" value="idle-idle">Idle</option>';
              selectTemplate= selectTemplate+'<option class="group-item" value="offline-offline">Non tracking</option>';
          }*/
        ...
        selectGroup: function(group) {
        	if ($L.isString(group) && group !== 'All') {
                ...
                var filterFunction = function(target) {
                	var result = false;
                	if (group === target.getAttribute("group") || (target.getAttribute("status")+'-'+target.getAttribute("status")) == group) {
                        result = true;
                    }
                	return result;
                };
