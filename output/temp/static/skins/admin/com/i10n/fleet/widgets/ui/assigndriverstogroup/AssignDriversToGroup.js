(function() {
     ...
     * @author dharma
     */
    $W.AssignDriversToGroup = function(el, oArgs) {
        $W.AssignDriversToGroup.superclass.constructor.call(this, el, oArgs);
    ...
    $L.extend($W.AssignDriversToGroup, $W.Assignment, {
        ...
        saveChanges: function(oInput) {
           ...
           oRequestParams.groupId = oInput.groupID;
             ...
             */
            $U.Connect.asyncRequest('POST', "@APP_CONTEXT@/form/admin/?command_type=assigndriver_group", this.oSaveCallback, $U.MapToParams(oRequestParams));
    ...
    $L.augmentObject($W.AssignDriversToGroup.prototype, {
        /*Defining call backs*/
        oSaveCallback: {
            success: function(o) {
            },
        ...
        oVehicleDataReceiptCallBack: {
            success: function(o) {
                ...
                if (!oResponse.adminmanage.groupmanage.Error) {
                    ...
                    this.reloadList(elList, sTemplate, oResponse.adminmanage.groupmanage.groupdrivers.assigned, function(el1, el2) {
                    ...
                    this.reloadList(elList, sTemplate, oResponse.adminmanage.groupmanage.groupdrivers.vacant, function(el1, el2) {
                        if (el1.name == el2.name) 
                            return 0;
                        else if (el1.name > el2.name) 
                            return 1;
                    ...
                    this.showAutoCompleteForAssignDriverToGroups();
        ...
        onGroupSelected: function(oEvent, oArgs) {
            ...
            $U.Connect.asyncRequest("GET", "@APP_CONTEXT@/view/controlpanel/?markup=AdminManage&debug=true&module=/blocks/json&data=view&subpage=group&dataView=assignmentdriver&groupID=" + groupID, this.oVehicleDataReceiptCallBack, null);
            /*
             * Preventing possible memory leaks
             */
            groupID = null;
